    ...
    def __init__(self, config: ConversationConfig):
        ...
        if self.config.llm_provider == "openrouter":
            if HAS_OPENROUTER:
                ...
                print(
                    "Warning: OpenRouter provider selected but openrouter package not installed."
                )
    ...
    def chat(
        ...
        if self.config.llm_provider == "openrouter" and self.openrouter_client:
            ...
            try:
                ...
                # Validate that choices_data is non-empty
                if not choices_data:
                    raise ValueError(
                        f"Invalid LLM response: no choices returned. "
        ...
        while iterations < self.config.max_tool_iterations:
            ...
            try:
                ...
                for tool_call in tool_calls:
                    ...
                    if self.config.show_tool_calls:
                        print(
                            f"  → {tool_name}({json.dumps(arguments, indent=2)[:100]}...)"
                        )
                ...
                # Handle empty choices error from LLM client
                if "no choices returned" in str(e):
                    if self.config.verbose:
                        print(f"\n[Warning] LLM returned empty response: {e}")
                    # If we get empty choices, break out of tool loop and try final response
                    break
                else:
                    raise

        # If we hit max iterations or got empty choices, get a final response without tools
        try:
            response = self.llm.chat(self.messages, tools=None)
            return response["choices"][0]["message"].get("content", "")
        except ValueError as e:
            if "no choices returned" in str(e):
                error_msg = f"LLM failed to generate a valid response: {e}"
                if self.config.verbose:
                    print(f"\n[Error] {error_msg}")
                return error_msg
            else:
        ...
        while True:
            ...
            if user_input.startswith("/"):
                ...
                if command == "/quit":
                    ...
                    if query:
                        result = manager.tool_handler.execute_tool(
                            "recall", {"query": query, "limit": 5}
                        )
...
Examples:
  ...
  python conversation.py --db-host localhost --db-name my_memory
    ...
    parser.add_argument("--model", "-m", default="llama3.2", help="Model name to use")
    parser.add_argument(
    ...
    parser.add_argument(
        "--db-port", type=int, default=env_db_port, help="Database port"
    )
    ...
    parser.add_argument(
        "--db-password", default=env_db_password, help="Database password"
    )
    ...
    # Display options
    parser.add_argument(
        "--hide-memories", action="store_true", help="Hide retrieved memories display"
    )
    parser.add_argument(
        "--hide-tool-calls", action="store_true", help="Hide tool call display"
    )
